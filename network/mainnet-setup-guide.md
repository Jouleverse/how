...
**【注意：下面，根据你是要搭建见证节点还是记账节点，在“选项一”和“选项二”两个里面选一个即可！】**

## 选项一：启动见证节点

**【注意：如果是升级节点，那么要先执行命令，停止节点的运行：```sudo docker stop jouleverse-mainnet```】**


一条命令清除所有已停止的容器（当遇到提示容器已存在错误时可随时执行）：```sudo docker container prune```
